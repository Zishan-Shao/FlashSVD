...
def _unwrap_first_tensor(value):
    candidate = value
    for _ in range(8):
...
def _can_use_manual_llama_decode_step(model) -> bool:
    inner = getattr(model, "model", None)
    return all(
        getattr(inner, attr, None) is not None
        for attr in ("embed_tokens", "layers", "norm", "rotary_emb")
    ) and getattr(model, "lm_head", None) is not None


def _manual_llama_decode_step(
    model,
    ...
    *,
    ...
    for decoder_layer in inner.layers[: model.config.num_hidden_layers]:
        ...
        hidden_states = _unwrap_first_tensor(layer_outputs)
...
def ppl_eval(
    ...
    *,
    ...
    for ds_name in datasets:
        print(f"[{ds_name}] Loading and tokenizing test split...")
        ...
        print(f"[{ds_name}] Running PPL over {n_seq} sequences | seqlen={model_seq_len} | batch={batch_size}")
...
def decode_kvcache_eval(
    ...
    *,
    ...
    if lowrank_cache:
        from runtime.cache.attn_legacy_sparse_kv import FlashSVDLegacySparseKVCache
        ...
        cache = FlashSVDLegacySparseKVCache(
            model.config,
        ...
        if layers is not None and len(layers) > 0:
            ...
            if attn0 is not None:
                ...
                if lowrank_cache:
                    ...
                    if baseline_env:
                        tag = "FlashSVD-attn-legacy-v1.5-sparsekv-baseline"
                    else:
                        try:
                        ...
                        if use_mha_stream:
                            tag = "FlashSVD-attn-legacy-sparsekv-mha-stream"
                        elif auto_mha_sdpa_env and (not force_kernel_env) and rep == 1 and R >= thr_sdpa:
                            tag = "FlashSVD-attn-legacy-sparsekv-auto-sdpa"
                        elif force_kernel_env:
                            tag = "FlashSVD-attn-legacy-sparsekv-forced-kernel"
                        else:
                            tag = "FlashSVD-attn-legacy-v1.5-sparsekv"
    ...
    use_manual_flashsvd_decode = bool(flashsvd_dense_cache) and _can_use_manual_llama_decode_step(model)

    def _decode_step(tokens: torch.Tensor, cache_pos: torch.Tensor):
        if use_manual_flashsvd_decode:
            return _manual_llama_decode_step(
                model,
                tokens,
    ...
    with torch.inference_mode():
        ...
        try:
            ...
            for t in range(int(max(0, warmup))):
                ...
                out = _decode_step(next_token, cache_pos)
                next_token = out.logits[:, -1, :].argmax(dim=-1, keepdim=True)
