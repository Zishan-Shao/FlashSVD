...
class FlashSVDV15DenseKVCache(Cache):
    """Dense KV cache with FA2-friendly layout [B, S, Hk, Dh]. This is specifically for llama-like models, given earlier models use [B, Hk, S, Dh]
    ...
    def _ensure_layer_spec(self, layer_idx: int, *, h: int, dh: int) -> None:
        layer_idx = int(layer_idx)
        h = int(h)
        dh = int(dh)
        ...
        if self._layer_initialized[layer_idx]:
            raise RuntimeError(
                f"FlashSVDV15DenseKVCache shape changed after init for layer {layer_idx}: "
    ...
    def _ensure_layer_shape(self, layer_idx: int, key_states: torch.Tensor, value_states: torch.Tensor) -> None:
        ...
        self._ensure_layer_spec(layer_idx, h=h, dh=dh)
    ...
    def clone_for_autotune(
        ...
        *,
        ...
        layer_idx = int(layer_idx)
        batch_size = int(batch_size)
        layer_device_map = {idx: dev for idx, dev in enumerate(self._layer_device)}
        cloned = type(self)(
            self._config,
            max_batch_size=self._max_batch_size,
            max_cache_len=self._max_cache_len,
            device=self._layer_device[0] if self._layer_device else None,
            dtype=self._dtype,
            layer_device_map=layer_device_map,
        )
        cloned._seen_tokens = int(self._seen_tokens)

        h = int(self._layer_heads[layer_idx])
        dh = int(self._layer_head_dim[layer_idx])
        if h > 0 and dh > 0:
            cloned._ensure_layer_spec(layer_idx, h=h, dh=dh)
            cloned.key_cache[layer_idx][:batch_size].copy_(self.key_cache[layer_idx][:batch_size])
            cloned.value_cache[layer_idx][:batch_size].copy_(self.value_cache[layer_idx][:batch_size])
            cloned._layer_initialized[layer_idx] = bool(self._layer_initialized[layer_idx])
        return cloned

    def prepare_fa2_direct_write_step(
        self,
        layer_idx: int,
        *,
        batch_size: int,
        ...
        self._ensure_layer_spec(layer_idx, h=int(num_heads), dh=int(head_dim))
        kc_full = self.key_cache[layer_idx]
        vc_full = self.value_cache[layer_idx]
        device = kc_full.device

        if cache_position is None:
            start = int(self.get_seq_length())
        elif torch.is_tensor(cache_position):
            pos = cache_position.to(device=device, dtype=torch.int64).reshape(-1)
            if int(pos.numel()) == 0:
                return None
            if int(pos.numel()) == 1:
                ...
                if bool(torch.any(pos != first).item()):
                    return None
                start = first
        else:
            start = int(cache_position)

        if start < 0 or (start + q_len) > int(self._max_cache_len):
            raise IndexError(
                f"FlashSVDV15DenseKVCache write position out of range: start={start} q_len={q_len} max={self._max_cache_len}"
            )

        kc = kc_full[:batch_size]
        vc = vc_full[:batch_size]
        cache_seqlens = self._get_cache_seqlens_buffer(layer_idx, batch_size)
        cache_seqlens.fill_(int(start + q_len))
        self._layer_initialized[layer_idx] = True
        return (
            kc,
            vc,
            kc[:, start : start + q_len],
            vc[:, start : start + q_len],
            cache_seqlens,
        )
...
FlashSVDDenseKVCache = FlashSVDV15DenseKVCache
