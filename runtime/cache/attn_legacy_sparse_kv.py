...
class FlashSVDLegacySparseKVCache(Cache):
    ...
    def update(
        ...
        if int(kc.shape[-1]) != k_rank or int(vc.shape[-1]) != v_rank:
            if self._layer_initialized[layer_idx]:
                raise RuntimeError(
                    "FlashSVDLegacySparseKVCache rank changed after initialization for layer "
...
LowRankKVCache = FlashSVDLegacySparseKVCache
