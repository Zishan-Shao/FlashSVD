...
In this case, we have the svd_kv_inference, this script is expected to have a real
inference with the SVD KV Cache.
"""
...
# =========================
# User prompt generation and inference
# =========================
def generate_user_prompt(tokenizer: AutoTokenizer, length: int = 256) -> str:
    """
    Generate a coherent 256-token prompt for GPT-2 and save it to prompt_exp.txt
    """
    # Create a meaningful prompt that GPT-2 can continue
    base_prompt = (
    ...
    # Tokenize and adjust to exactly the desired length
    tokens = tokenizer.encode(base_prompt)
    
    if len(tokens) > length:
        tokens = tokens[:length]
    elif len(tokens) < length:
        # Pad with additional coherent text
        padding_text = (
        ...
        # Repeat padding until we reach the desired length
...
def inference_with_user_prompt(
    ...
    """
    Perform prefilling and decoding with the user prompt
    """
    ...
    # Prefilling phase: process the entire prompt at once
    ...
    print(f"[PREFILLING] Completed in {prefill_time*1000:.2f}ms")
    
    # Decoding phase: generate tokens one by one
    print(f"\n[DECODING] Generating {generate_tokens} tokens...")
    ...
    tokens_per_sec = generate_tokens / max(decode_time, 1e-6)
    
    print(f"\n=== Performance Metrics ===")
    print(f"Prefilling time: {prefill_time*1000:.2f}ms")
    print(f"Decoding time: {decode_time*1000:.2f}ms ({tokens_per_sec:.2f} tokens/sec)")
    print(f"Total time: {total_time*1000:.2f}ms")
    print(f"Peak memory: {peak_memory:.1f} MiB")
    
    print(f"\n=== Generated Text ===")
    print(f"Generated continuation ({generate_tokens} tokens):")
...
def main():
    ...
    if args.user_prompt:
        ...
        # Perform inference with SVD model
        print("\n--- SVD Model Inference ---")
        svd_output = inference_with_user_prompt(
            svd_model, tok, prompt_text, args.generate_tokens, device
        )
        
        # Optionally compare with dense model
        if args.compare_dense:
            print("\n--- Dense Model Inference ---")
            dense_output = inference_with_user_prompt(
                dense, tok, prompt_text, args.generate_tokens, device
            )
        
        return  # Exit after user prompt inference
...
'''
# Basic user prompt inference
python profile_svd_kv_infer.py --user-prompt --generate-tokens 50

# Compare SVD vs Dense model
python profile_svd_kv_infer.py --user-prompt --generate-tokens 100 --compare-dense --rank-ratio-attn 0.9 --rank-ratio-mlp 1.0

# With custom settings
python profile_svd_kv_infer.py --user-prompt --generate-tokens 75 --validate --rank-ratio-attn 0.5
'''
