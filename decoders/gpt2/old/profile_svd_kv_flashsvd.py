...
class LinearSVDBlock(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, past_key_value=None, use_cache=False, **kwargs):
        ...
        # ─── Attention via FlashSVDAttn with low-rank KV-cache ───────────────
        x = self.ln1(hidden_states)  # [B, Q, D]

        # Build low-rank Pq,Pk,Pv per head
        ...
        Pq_cur = torch.stack(Pq_list, dim=1)  # [B, H, Q, R]
        Pk_new = torch.stack(Pk_list, dim=1)  # [B, H, Q, R]
        Pv_new = torch.stack(Pv_list, dim=1)  # [B, H, Q, R]

        # Append KV-cache (low-rank)
        if past_key_value is not None and len(past_key_value) > 0:
            past_Pk, past_Pv = past_key_value
            Pk = torch.cat([past_Pk, Pk_new], dim=-2)  # [B, H, K, R]
            Pv = torch.cat([past_Pv, Pv_new], dim=-2)  # [B, H, K, R]
        else:
            Pk, Pv = Pk_new, Pv_new
        kv_seq_len = Pk.size(-2)
        present_key_value = (Pk, Pv) if use_cache else None

        # Expand bases and biases per head
        Rq = self.q_U.shape[-1]
        Vq_full = self.q_U.transpose(1, 2).unsqueeze(0).expand(batch_size, -1, -1, -1).contiguous()
        Vk_full = self.k_U.transpose(1, 2).unsqueeze(0).expand(batch_size, -1, -1, -1).contiguous()
        Vv_full = self.v_U.transpose(1, 2).unsqueeze(0).expand(batch_size, -1, -1, -1).contiguous()
        bq_full = self.q_bias.unsqueeze(0).expand(batch_size, -1, -1).contiguous()
        bk_full = self.k_bias.unsqueeze(0).expand(batch_size, -1, -1).contiguous()
        bv_full = self.v_bias.unsqueeze(0).expand(batch_size, -1, -1).contiguous()

        # Pad Pq to global length kv_seq_len so last seq_len positions correspond to current queries
        if kv_seq_len > seq_len:
            pad_len = kv_seq_len - seq_len
            zeros_pad = torch.zeros(batch_size, self.n_heads, pad_len, Rq, device=hidden_states.device, dtype=Pq_cur.dtype)
            Pq_full = torch.cat([zeros_pad, Pq_cur], dim=-2)  # [B,H,K,R]
        else:
            Pq_full = Pq_cur

        # Build key mask [B,1,1,K] expected by flash_svd_attention
        if attention_mask is not None and attention_mask.dim() == 2:
            key_mask = attention_mask[:, :kv_seq_len].to(torch.bool)
            mask4 = key_mask.view(batch_size, 1, 1, kv_seq_len)
        elif attention_mask is not None and attention_mask.dim() == 4:
            am4 = attention_mask[..., -kv_seq_len:]
            if am4.dtype.is_floating_point:
                am_bool = am4 > 0
            else:
                am_bool = am4.to(torch.bool)
            if am_bool.size(2) != 1:
                am_bool = am_bool.any(dim=2, keepdim=True)
            if am_bool.size(1) != 1:
                am_bool = am_bool.any(dim=1, keepdim=True)
            mask4 = am_bool
        else:
            mask4 = torch.ones(batch_size, 1, 1, kv_seq_len, device=hidden_states.device, dtype=torch.bool)

        # Low-rank causal attention in PyTorch (KV-aware, per-query causal)
        # Build per-head outputs
        dh = self.head_dim
        scale = 1.0 / math.sqrt(dh)
        attn_heads = []
        # Use only the last seq_len queries from Pq_full for current step
        Pq_tail = Pq_full[:, :, -seq_len:, :]  # [B,H,Q,R]
        for h in range(self.n_heads):
            Vq_h = self.q_U[h].T  # [R, dh]
            Vk_h = self.k_U[h].T  # [R, dh]
            Vv_h = self.v_U[h].T  # [R, dh]
            Y = Vq_h @ Vk_h.T      # [R, R]
            logits = (Pq_tail[:, h] @ Y) @ Pk[:, h].transpose(1, 2)  # [B,Q,K]
            # Add column bias from bq: bq @ (Pk Vk)^T = (Pk Vk @ bq)^T
            Tk = Pk[:, h] @ Vk_h            # [B,K,dh]
            col_bias = Tk @ self.q_bias[h]  # [B,K]
            logits = logits + col_bias.unsqueeze(1)
            # Causal mask for each query position relative to K
            Qlen, Klen = seq_len, kv_seq_len
            past_len = Klen - Qlen
            ...
            causal_mask = causal.view(1, Qlen, Klen)
            # Key padding mask from mask4 (True=keep)
            if mask4 is not None:
                key_keep = mask4.view(batch_size, 1, 1, Klen).expand(-1, Qlen, -1, -1).squeeze(2)
            else:
                key_keep = torch.ones(batch_size, Qlen, Klen, device=hidden_states.device, dtype=torch.bool)
            keep_mask = key_keep & causal_mask  # [B,Q,K]
            # Set masked positions to a large negative, but avoid all-masked rows → zeros
            logits = logits.masked_fill(~keep_mask, float('-1e9'))
            no_valid = ~keep_mask.any(dim=-1)  # [B,Q]
            logits = torch.where(no_valid.unsqueeze(-1), torch.zeros_like(logits), logits)
            attn_prob = torch.softmax(logits * scale, dim=-1)
            # Zero probabilities for rows that had no valid keys
            attn_prob = torch.where(no_valid.unsqueeze(-1), torch.zeros_like(attn_prob), attn_prob)
