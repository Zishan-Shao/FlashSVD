...
class LinearSVDBlock(nn.Module):
    ...
    def __init__(self, linear_block: LinearGPT2Block, rank_attn: int, rank_ff: int, rank_out: int):
        ...
        def decompose_per_head(weight, rank):
            ...
            for h in range(self.n_heads):
                ...
                # Use QR decomposition instead of SVD for better numerical stability
                Q, R = torch.linalg.qr(head_weight)
                r = min(rank, Q.shape[1], R.shape[0])
                
                # Create low-rank factors using QR
        ...
        self.fc1_U = nn.Parameter(Q[:, :r_fc1].to(fc1_weight.dtype))  # [intermediate, r_fc1]
        self.fc1_V = nn.Parameter(R[:r_fc1, :].to(fc1_weight.dtype))  # [r_fc1, d_model]
        
        # Clip extreme values
        self.fc1_U.data = torch.clamp(self.fc1_U.data, min=-1e6, max=1e6)
        self.fc1_V.data = torch.clamp(self.fc1_V.data, min=-1e6, max=1e6)
        ...
        self.fc2_U = nn.Parameter(Q[:, :r_fc2].to(fc2_weight.dtype))  # [d_model, r_fc2]
        self.fc2_V = nn.Parameter(R[:r_fc2, :].to(fc2_weight.dtype))  # [r_fc2, intermediate]
        
        # Clip extreme values
        self.fc2_U.data = torch.clamp(self.fc2_U.data, min=-1e6, max=1e6)
        self.fc2_V.data = torch.clamp(self.fc2_V.data, min=-1e6, max=1e6)
    ...
    def forward(self, hidden_states, attention_mask=None, past_key_value=None, use_cache=False, **kwargs):
        ...
        k_t = k.transpose(1, 2).contiguous()
        v_t = v.transpose(1, 2).contiguous()
        # Build matrix bias including causal with offset for KV cache
        past_len = kv_seq_len - seq_len
        i_idx = torch.arange(seq_len, device=hidden_states.device).view(seq_len, 1)
        j_idx = torch.arange(kv_seq_len, device=hidden_states.device).view(1, kv_seq_len)
        causal_allow = (j_idx <= (past_len + i_idx))
        bias = torch.zeros(batch_size, 1, seq_len, kv_seq_len, device=hidden_states.device, dtype=torch.float32)
        bias.masked_fill_(~causal_allow.view(1, 1, seq_len, kv_seq_len), float('-inf'))
        if attention_mask is not None:
            if attention_mask.dim() == 2:
        ...
        K = K * kmask.to(K.dtype)
        V = V * kmask.to(V.dtype)
        attn_out = flash_attn_triton_unified(Q, K, V, qmask4d, BLOCK_M=16)
        # Reshape and project output using SVD factors
        attn_output = attn_out.transpose(1, 2).contiguous().view(batch_size, seq_len, d_model)
        attn_output = attn_output.to(hidden_states.dtype)
        ...
        ff_output = torch.clamp(ff_output, min=-1e6, max=1e6)  # Clip for stability
        ff_output = F.gelu(ff_output)            # GELU activation
...
if __name__ == "__main__":
    ...
    device     = "cuda"
    RANK_ATTN  = 64 // 2     # Full rank per head (head_dim = 64)
    RANK_FF    = 768 // 2   # Full rank for FFN (min(768, 3072) = 768) - exact reconstruction  
    RANK_WO    = 768 // 2   # Full rank for output projection [768, 768] - exact reconstruction  
