...
def _make_eval_additive_pad_mask(attn_2d: torch.Tensor) -> torch.Tensor:
    """
    Build a 4D additive mask [B,1,T,T] that masks out right-padded queries/keys.
    We DO NOT put causal there; the block will add causal itself. Values:
      0.0  -> keep
     -1e4  -> mask (safely < -1e3 threshold used inside the block)
    """
    # attn_2d: [B, T] with 1 for real tokens, 0 for pads (right-padded)
    B, T = attn_2d.shape
    device = attn_2d.device
    dtype = torch.float32  # will be cast inside the block to the layer dtype

    lengths = attn_2d.sum(dim=1)                    # [B]
    q_idx   = torch.arange(T, device=device).view(1, 1, T, 1)  # [1,1,T,1]
    k_idx   = torch.arange(T, device=device).view(1, 1, 1, T)  # [1,1,1,T]
    L       = lengths.view(B, 1, 1, 1)                         # [B,1,1,1]

    # Valid if both the query and key are within the per-example length
    valid_qk = (q_idx < L) & (k_idx < L)                       # [B,1,T,T]

    keep  = torch.zeros((B, 1, T, T), dtype=dtype, device=device)
    maskv = torch.full_like(keep, -1e4)
    return torch.where(valid_qk, keep, maskv)                  # [B,1,T,T]
...
def eval_perplexity_fullseq(model, loader, device):
    """
    Evaluation with right padding: trim inputs to the max non-pad length per batch
    and pass a 2D mask of that same length. This avoids the internal 'take last k'
    slicing that corrupts masking for right-padded batches.
    """
    model.eval()
    ...
    for i, batch in enumerate(loader):
        ...
        am2d      = batch["attention_mask"].to(device)  # [B,T] (1=real, 0=pad)
        B, T_full = input_ids.shape

        # Trim to the maximum non-pad length in this batch
        lengths = am2d.sum(dim=-1)                # [B]
        L = int(lengths.max().item())
        if L <= 1:
            # Nothing usable to score (or all pads) – skip this mini-batch
            continue

        ids = input_ids[:, :L]                    # [B,L]
        am  = am2d[:, :L]                         # [B,L]
        pos = torch.arange(L, device=device).unsqueeze(0).repeat(B, 1)  # [B,L]

        # Forward once, no cache; the block will add causal and use our 2D mask
        out = model(input_ids=ids,
                    attention_mask=am,
                    position_ids=pos,
                    use_cache=False)

        # Teacher-forced LM loss over non-pad tokens (standard left-shift)
        logits = out.logits[:, :-1, :].contiguous()   # [B,L-1,V]
        labels = ids[:, 1:].contiguous()              # [B,L-1]
        mask   = am[:, 1:].contiguous().bool()        # [B,L-1]
        ...
        if mask.any():
            v_logits = logits[mask].float()   # [N,V] — do CE in fp32
