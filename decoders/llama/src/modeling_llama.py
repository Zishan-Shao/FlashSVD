...
class LowRankLinear(nn.Module):
    ...
    """
    def __init__(self, in_features: int, out_features: int, rank: int, bias: bool = True, device=None, dtype=None, init: str = "factorized", compute_in_float32: bool = True):
        super().__init__()
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.compute_in_float32:
            ...
            tmp = nn.functional.linear(x, V, None)
            out = nn.functional.linear(tmp, U, bias)
    ...
    @staticmethod
    def from_linear(linear: nn.Linear, rank: int, *, factor_dtype: torch.dtype | None = None, compute_in_float32: bool = True) -> "LowRankLinear":
        ...
        lr = LowRankLinear(
            ...
            device=device,
            dtype=(factor_dtype or orig_dtype),
        ...
        for layer in self.model.layers:
            ...
            def maybe_replace_linear(module: nn.Module, attr: str, rank: Optional[int]):
                ...
                factor_dtype = {
                    ...
                    "bf16": torch.bfloat16,
                    "fp32": torch.float32,
                }.get(svd_dtype.lower(), lin.weight.dtype)
                lr = LowRankLinear.from_linear(lin, rank=rank, factor_dtype=factor_dtype, compute_in_float32=compute_in_float32)
