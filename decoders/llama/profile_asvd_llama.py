...
Decoding (dynamic cache path, Plan B):
...
class SVDLlamaBlock(nn.Module):
    ...
    LLaMA block:
      ...
      • PyTorch SDPA (causal)
      • RoPE with HF semantics (respects position_ids when provided)
    ...
    def forward(self, hidden_states, attention_mask=None, past_key_value=None,
        ...
        if use_cache and hasattr(past_key_value, "update"):
            ...
            if use_cache and isinstance(past_key_value, (tuple, list)) and len(past_key_value) == 2:
                ...
                present = (k_seq, v_seq) if use_cache and not hasattr(past_key_value, "update") else None
        ...
        # # Build additive mask with causal + padding
        # k_len = k_bhsd.size(-2)
        # bias = _build_full_bias(attention_mask, B, T_max, k_len, q_bhsd.device, q_bhsd.dtype)
        # Build / select additive mask that matches our (Q= T_max, K = k_len)
        # Build / select additive mask that matches our (Q = T_max, K = k_len)
        k_len = k_bhsd.size(-2)

        if attention_mask is not None and attention_mask.dim() == 4:
            # HF already gave us [B, 1, Q_all, K_all] with causal+pad baked in.
            ...
            # IMPORTANT: make sure bias has a fresh, aligned allocation
            # (slicing can leave a non-zero storage offset that breaks SDPA kernels).
            if (not bias.is_contiguous()) or (bias.storage_offset() != 0) or ((bias.data_ptr() % 16) != 0):
        ...
        if os.getenv("DEBUG_CACHE", "0") == "1" and (self.layer_idx in (None, 0)):
            try:
                print(f"[DEBUG] mask.shape={tuple(bias.shape)} off={getattr(bias, 'storage_offset', lambda:0)()} "
                    f"contig={bias.is_contiguous()} ptr%16={bias.data_ptr()%16}")
            except Exception:
                pass
...
def eval_perplexity_fullseq(model, loader, device):
    ...
    for i, batch in enumerate(loader):
        ...
        # Provide position_ids to match our RoPE exactly
        pos = torch.arange(batch["input_ids"].size(1), device=device).unsqueeze(0).repeat(batch["input_ids"].size(0), 1)
...
def profile_prefill_and_decode(model, tok: AutoTokenizer, device: str,
    ...
    t0 = time.perf_counter()
    out = model(input_ids=input_ids, attention_mask=attn_mask, position_ids=pos_ids_full, use_cache=True)
    if torch.cuda.is_available():
        torch.cuda.synchronize()
    ...
    for step in range(decode_tokens):
        ...
        out = model(input_ids=next_ids,
                    ...
                    cache_position=pos_next.squeeze(1),
...
if __name__ == "__main__":
    ...
    if MODE == "eval":
        ...
        if torch.cuda.is_available():
            ...
            torch.cuda.reset_peak_memory_stats()
            torch.cuda.synchronize()
        prompt_file = os.path.join(os.path.dirname(__file__), 'synthetic_prompt_256.txt')
...
# CUDA_VISIBLE_DEVICES=0 SVD_DTYPE=fp16 SVD_COMPUTE_FP32=1 DEBUG_CACHE=1 MAX_EVAL_SAMPLES=10 python profile_svd_kv_llama.py
