...
ASVD additions (decode path only):
  ...
  • Profiling reports measured KV MiB and an estimate that scales with rank_kv.

Usage:
  Evaluation:
    MODE=eval BATCH_SIZE=1 SEQ_LEN=512 python profile_svd_kv_llama.py

  Decoding (legacy KV path forced by default; required for ASVD):
...
class SVDLlamaBlock(nn.Module):
    ...
    def _apply_rope(self, q_bhtd, k_bhtd, position_ids, position_embeddings=None):
        ...
        try:
            q_rot, k_rot = apply_rotary_pos_emb(q_bhtd, k_bhtd, cos, sin, position_ids=position_ids)
        except TypeError:
            q_rot, k_rot = apply_rotary_pos_emb(q_bhtd, k_bhtd, cos, sin)
    ...
    def _apply_rope_single(self, x_bhtd, position_ids=None, seq_len=None):
        """
        === ASVD ===
        Apply RoPE to a single tensor [B,H,T,Dh] with its own length/positions.
        """
        B, H, T, Dh = x_bhtd.shape
        # Build cos/sin for this T (or given seq_len)
        target_len = int(seq_len) if seq_len is not None else T
        try:
            sig = inspect.signature(self.rotary_emb.forward)
            if "position_ids" in sig.parameters and position_ids is not None:
                cos, sin = self.rotary_emb(x_bhtd, position_ids=position_ids)
            elif "seq_len" in sig.parameters:
        ...
        # IMPORTANT: pass position_ids so we don't broadcast to a longer time axis
        try:
            xr, _ = apply_rotary_pos_emb(x_bhtd, x_bhtd, cos, sin, position_ids=position_ids)
        except TypeError:
            # Older HF without position_ids in apply_rotary_pos_emb → gather manually
            if position_ids is not None and cos.size(-2) >= x_bhtd.size(-2):
                cos_b = cos.expand(x_bhtd.size(0), 1, cos.size(-2), cos.size(-1))
                sin_b = sin.expand(x_bhtd.size(0), 1, sin.size(-2), sin.size(-1))
                idx = position_ids.unsqueeze(1).unsqueeze(-1).expand(-1, 1, -1, cos.size(-1))
                cos_sel = torch.gather(cos_b, -2, idx)
                sin_sel = torch.gather(sin_b, -2, idx)
                xr, _ = apply_rotary_pos_emb(x_bhtd, x_bhtd, cos_sel, sin_sel)
            else:
                # Final fallback: trim caches to current T
                cos_t = cos[..., :x_bhtd.size(-2), :]
                sin_t = sin[..., :x_bhtd.size(-2), :]
                xr, _ = apply_rotary_pos_emb(x_bhtd, x_bhtd, cos_t, sin_t)
    ...
    def forward(self, hidden_states, attention_mask=None, past_key_value=None,
        ...
        if not use_cache:
            ...
            if self.asvd_enabled:
                # === ASVD === cache low-rank Pk,Pv; reconstruct K,V on the fly
                ...
                # 2) Extend cache (legacy tuple path). We deliberately ignore HF DynamicCache here.
                if isinstance(past_key_value, (tuple, list)) and len(past_key_value) == 2:
                    pk_seq = torch.cat([past_key_value[0], Pk_new], dim=2)  # [B,Hk,K,T]→[B,Hk,K+T]
                    pv_seq = torch.cat([past_key_value[1], Pv_new], dim=2)
                    present = (pk_seq, pv_seq)
                elif past_key_value is None or (isinstance(past_key_value, list) and len(past_key_value) == 0):
                    pk_seq, pv_seq = Pk_new, Pv_new
                    present = (pk_seq, pv_seq)
                else:
                    # If a dynamic cache object sneaks in, fall back to starting a fresh tuple cache.
                    pk_seq, pv_seq = Pk_new, Pv_new
                    present = (pk_seq, pv_seq)
                ...
                k_len = k_seq.size(-2)
                # Absolute positions for keys: 0..k_len-1
                ...
                ctx = (torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=False)
                       if (bias is not None and q_bhsd.is_cuda) else nullcontext())
                with ctx:
        ...
        if use_cache:
            # present is set in both ASVD and non-ASVD branches
            return (out, present)
        return (out,)
...
def profile_prefill_and_decode(model, tok: AutoTokenizer, device: str,
    ...
    force_legacy_kv = True if asvd_enable else (os.getenv("FORCE_LEGACY_KV", "1") == "1")
    ...
    # --- Option A (ASVD): seed legacy tuples with zero-length (Pk,Pv) to block DynamicCache
    if asvd_enable and force_legacy_kv:
        cfg = model.config
        n_heads = int(getattr(cfg, 'num_attention_heads'))
        n_kv_heads = int(getattr(cfg, 'num_key_value_heads', n_heads))
        r = int(rank_kv)
        empty_pk = torch.empty(batch_size, n_kv_heads, 0, r, dtype=svd_dtype_env, device=device)
        empty_pv = torch.empty(batch_size, n_kv_heads, 0, r, dtype=svd_dtype_env, device=device)
        init_past = [(empty_pk.clone(), empty_pv.clone()) for _ in range(int(getattr(cfg, 'num_hidden_layers')))]
    else:
        # original: list of Nones (may fall back to dynamic on newer HF)
        init_past = [None] * model.config.num_hidden_layers if force_legacy_kv else None
