...
def flashsvd_rope_sdpa(
    # Pq,Pk,Pv rank-space activations
    Pq_ptr, Pk_ptr, Pv_ptr,
    # Vq,Vk,Vv factor weights (R x Hdh) laid out as [R, H, dh] logically
    Vq_ptr, Vk_ptr, Vv_ptr,
    # Optional biases per-head in head-space (H*dh) or 0
    ...
    sPk_b, sPk_m, sPk_r,
    sPv_b, sPv_m, sPv_r,
    sVq_r, sVq_hd,    # Vq is [R, H*dh]
    sVk_r, sVk_hd,
    sVv_r, sVv_hd,
    sbq_hd, sbk_hd, sbv_hd,  # may be 0 if no bias
    ...
    for nk in range(0, M, BN):
        ...
        # ---- Masks ----
        if HAS_PAD:
            # keys-only semantics for 2D padding mask: 1=valid, 0=pad
        ...
        for d0 in range(0, dh, BDH):
            ...
            for r0 in range(0, R, BR):
                ...
                v_blk_src = tl.load(Pv_ptr + bid*sPv_b + offs_n[:, None]*sPv_m + r[None, :]*sPv_r,
                                    mask=(offs_n[:, None] < M) & mask_r[None, :], other=0.0)
...
class FlashSVDRoPEAttention(nn.Module):
    ...
    def forward(self,
        ...
        if has_pad:
            ...
            sPM_b = sPM_m = 0
        if has_add:
            # IMPORTANT: use the additive-mask's own strides
        ...
        flashsvd_rope_sdpa[grid](
            ...
            sO_b, sO_h, sO_m, sO_dh,
            sPM_b, sPM_m,
            sAM_b, sAM_mq, sAM_mk,          # <-- correct additive-mask strides
...
class ExplicitSVDWithRoPEKernelBlock(nn.Module):
    ...
    def forward(
        ...
        O = self.flash(
            QKVFactors(
                Pq=Pq, Pk=Pk, Pv=Pv,
                Vq=self.Vq_proj.weight.t().contiguous(),  # [R, D]
...
def _reference_sdpa_with_rope(Pq, Pk, Pv, Vq, Vk, Vv, bq, bk, bv, cos, sin,
    ...
    if attention_mask_2d is not None:
        # attention_mask_2d: [B,M] with 1=valid, 0=pad
        am = attention_mask_2d.to(torch.bool)
        # mask out invalid (either q or k). Typically we only need to mask keys.
        # Here we follow your kernel: require both sides valid; else -inf.
    ...
    Oref = torch.einsum("bhmn,bhnd->bhmd", attn, V)  # [B,H,M,dh]
    return Oref
