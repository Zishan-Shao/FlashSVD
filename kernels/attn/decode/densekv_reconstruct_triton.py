...
@triton.jit
def _reconstruct_qkv_token_shared_hetero_kernel(
    Pq_ptr, Pk_ptr, Pv_ptr,
    Vq_ptr, Vk_ptr, Vv_ptr,
    bq_ptr, bk_ptr, bv_ptr,
    Q_ptr, K_ptr, V_ptr,
...
def reconstruct_qkv_token_shared(
    ...
    *,
    ...
    if Pq.ndim != 2 or Pk.ndim != 2 or Pv.ndim != 2:
        raise ValueError(
            f"Expected Pq/Pk/Pv to be 2D [B,R*], got {tuple(Pq.shape)}/{tuple(Pk.shape)}/{tuple(Pv.shape)}"
        )
    if Vq.ndim != 3 or Vk.ndim != 3 or Vv.ndim != 3:
        raise ValueError(
            f"Expected Vq/Vk/Vv to be 3D [H|Hk,R,Dh], got {tuple(Vq.shape)}/{tuple(Vk.shape)}/{tuple(Vv.shape)}"
...
def reconstruct_qkv_token_shared_hetero(
    Pq: torch.Tensor,
    Pk: torch.Tensor,
    Pv: torch.Tensor,
    Vq: torch.Tensor,
    ...
    *,
    ...
    """
    Decode-only token reconstruction for shared global-rank factors with hetero q/k/v ranks.
    ...
    """
    if not (Pq.is_cuda and Pk.is_cuda and Pv.is_cuda and Vq.is_cuda and Vk.is_cuda and Vv.is_cuda):
        raise ValueError("reconstruct_qkv_token_shared_hetero expects all tensors on CUDA.")
    if Pq.ndim != 2 or Pk.ndim != 2 or Pv.ndim != 2:
        raise ValueError(
            f"Expected Pq/Pk/Pv to be 2D [B,R*], got {tuple(Pq.shape)}/{tuple(Pk.shape)}/{tuple(Pv.shape)}"
        )
    ...
    Bv, Rv = Pv.shape
    if Bk != B or Bv != B:
        raise ValueError("Mismatched batch dimension across Pq/Pk/Pv.")
    H = int(Vq.shape[0])
    Hk = int(Vk.shape[0])
    Dh = int(Vq.shape[2])
    if Vq.shape[1] != Rq:
        raise ValueError(f"Expected Vq rank {Rq}, got {int(Vq.shape[1])}")
    if Vk.shape[1] != Rk:
        raise ValueError(f"Expected Vk rank {Rk}, got {int(Vk.shape[1])}")
    if Vv.shape[1] != Rv:
        raise ValueError(f"Expected Vv rank {Rv}, got {int(Vv.shape[1])}")
    if Vk.shape[2] != Dh or Vv.shape[2] != Dh:
        raise ValueError("All bases must share the same head_dim.")
    ...
    _reconstruct_qkv_token_shared_hetero_kernel[grid](
        Pq, Pk, Pv,
        Vq, Vk, Vv,
        bq if bq is not None else Q,
        bk if bk is not None else K,
...
def pack_qkv_shared_bases(
    ...
    """
    Prepack decode bases to GEMM-friendly [R*, H*Dh] / [R*, Hk*Dh] layout.
    ...
    Vq_flat = Vq.permute(1, 0, 2).reshape(Rq, H * Dh).contiguous()
    Vk_flat = Vk.permute(1, 0, 2).reshape(Rk, Hk * Dh).contiguous()
    Vv_flat = Vv.permute(1, 0, 2).reshape(Rv, Hk * Dh).contiguous()
...
def reconstruct_qkv_token_shared_prepacked(
    ...
    *,
    ...
    """
    Token reconstruction using GEMM/GEMV-friendly prepacked bases.
    ...
    if Bk != B or Bv != B:
        raise ValueError("Mismatched batch dimension across Pq/Pk/Pv.")
    if Vq_flat.shape != (Rq, H * Dh):
        raise ValueError(f"Expected Vq_flat shape {(Rq, H * Dh)}, got {tuple(Vq_flat.shape)}")
    if Vk_flat.shape != (Rk, Hk * Dh):
        raise ValueError(f"Expected Vk_flat shape {(Rk, Hk * Dh)}, got {tuple(Vk_flat.shape)}")
    if Vv_flat.shape != (Rv, Hk * Dh):
        raise ValueError(f"Expected Vv_flat shape {(Rv, Hk * Dh)}, got {tuple(Vv_flat.shape)}")
