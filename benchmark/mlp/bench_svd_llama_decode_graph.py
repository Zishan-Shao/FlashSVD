...
from benchmark.mlp.legacy_swiglu import flashsvd_ffn_swiglu
...
def run_kernel_bench(layer, hidden_states: torch.Tensor, warmup: int, runs: int):
    ...
    # This benchmark targets the old shared-P SwiGLU experiment shape rather than
    # the current exact production MLP path. Reconstruct the legacy factor layout
    # directly from today's MLP weights instead of depending on removed helpers.
    gate_u = mlp.gate_u_proj.weight.t().to(device=hidden_states.device, dtype=hidden_states.dtype).contiguous()
    up_u = mlp.up_u_proj.weight.t().to(device=hidden_states.device, dtype=hidden_states.dtype).contiguous()
    down_v = mlp.down_v_proj.weight.t().to(device=hidden_states.device, dtype=hidden_states.dtype).contiguous()
    down_u = mlp.down_u_proj.weight.t().to(device=hidden_states.device, dtype=hidden_states.dtype).contiguous()
    V1 = torch.cat([gate_u, up_u], dim=1).contiguous()
    U2 = down_v
    V2 = down_u
    b1 = torch.zeros((2 * gate_u.shape[1],), device=hidden_states.device, dtype=hidden_states.dtype)
    b2 = torch.zeros((down_u.shape[1],), device=hidden_states.device, dtype=hidden_states.dtype)
