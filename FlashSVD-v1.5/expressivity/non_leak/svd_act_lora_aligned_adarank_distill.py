...
from utils.perf_profile import CudaMemProfiler
...
CUDA_VISIBLE_DEVICES=1 python -u expressivity/svd_act_lora_aligned_adarank.py   --model meta-llama/Llama-2-7b-hf \
                    ...
                    --bucket_props LM:0.3,INST:0.4,MCQ:0.15,MATH:0.15 \
                    --bucket_lm_datasets c4_stream,wikitext2 --c4_stream_train_docs 128 \
                    --bucket_inst_datasets Open-Orca/SlimOrca \
                    --bucket_mcq_datasets race,sciq,social_iqa \
                    --bucket_math_datasets mathqa   --bucket_loss_weights LM:1.0,INST:1.0,MCQ:0.8,MATH:1.0 \
...
cd /home/zs89/FlashSVDTrain
mkdir -p checkpoints/non_leak
export HF_TOKEN=hf_xxx   # 需要访问 Llama-2 的话

CUDA_VISIBLE_DEVICES=1 python -u expressivity/non_leak/svd_act_lora_aligned_adarank_opt.py \
  --model meta-llama/Llama-2-7b-hf \
  --keep_ratio 0.4 --whitening_factorization svd \
  --whitening_nsamples 512 --lora_nsamples 8192 \
  --lora_rank 32 --lora_alpha 32 --epochs 2 --lr 2e-4 \
  --train_batch_size 1 --gradient_checkpointing \
  --mix_buckets \
  --bucket_lm_datasets c4_stream,wikitext2,ptb --c4_stream_train_docs 256 \
  --bucket_inst_datasets Open-Orca/SlimOrca,tatsu-lab/alpaca \
  --bucket_mcq_datasets race,sciq,social_iqa \
  --bucket_math_datasets mathqa \
  --whitened_cache checkpoints/non_leak/llama2_7b_kr0.4_whiten.pt \
  --save_path checkpoints/non_leak/llama2_7b_kr0.4_actlora.pt \
  --force_recompress


CUDA_VISIBLE_DEVICES=1 python -u expressivity/non_leak/svd_act_lora_aligned_adarank_opt.py \
  --model meta-llama/Llama-2-7b-hf --keep_ratio 0.4 --whitening_factorization svd \
  --lora_rank 32 --lora_alpha 32 --epochs 2 --lr 2e-4 --train_batch_size 8 --gradient_checkpointing --mix_buckets \
  --bucket_lm_datasets c4_stream,wikitext2,ptb --c4_stream_train_docs 256 \
  --bucket_inst_datasets HuggingFaceH4/ultrachat_200k,alpaca --bucket_mcq_datasets race,sciq,social_iqa \
  --bucket_math_datasets gsm8k,aqua_rat --whitened_cache checkpoints/non_leak/llama2_7b_kr0.4_whiten.pt \
  --save_path checkpoints/non_leak/llama2_7b_kr0.4_actlora.pt --force_recompress --profile_mem --profile_mem_every 20 --timing_out checkpoints/non_leak/timing.json \
  --kd_teacher_model same --kd_teacher_device cpu --kd_weight 0.5 --kd_temperature 2.0 --kd_epochs 1 --kd_lr 1e-4
...
def _split_hf_dataset_spec(spec: str) -> Tuple[str, Optional[str]]:
    """
    Split a dataset spec into (path, config) to support datasets that require a config name.
...
def _maybe_enable_gradient_checkpointing(model: nn.Module, enable: bool) -> bool:
    if not enable:
        return False
    ok = False
    try:
        # HF disables cache when using checkpointing; enforce to avoid warnings.
    ...
    try:
        ...
        ok = False
    try:
        # Needed for re-entrant checkpoint in some HF versions when only adapter weights require grad.
        if hasattr(model, "enable_input_require_grads"):
            model.enable_input_require_grads()
    except Exception:
        pass
    if ok:
        print("[Mem] Enabled HF gradient checkpointing for LoRA training.")
    else:
        print("[Mem] gradient_checkpointing_enable() not available on this model; skipping.")
    return ok
...
def _dtype_from_string(name: Optional[str], default: torch.dtype = torch.float32) -> torch.dtype:
    if name is None:
        return default
    s = str(name).strip().lower()
    if s in ("fp32", "float32", "f32"):
        return torch.float32
    if s in ("fp64", "float64", "f64", "double"):
...
def _safe_spd_cholesky(mat: torch.Tensor) -> torch.Tensor:
    mat = (mat + mat.t()) * 0.5
    try:
        return torch.linalg.cholesky(mat)
    except Exception:
        # Scale-aware diagonal jitter, then fallback to eigenvalue clipping.
        dmean = float(mat.diag().abs().mean().item())
        eps = 1e-6 * (dmean if dmean > 0.0 else 1.0)
        eye = torch.eye(mat.shape[0], device=mat.device, dtype=mat.dtype)
        mat_j = mat + eps * eye
        try:
            return torch.linalg.cholesky(mat_j)
        except Exception:
            w, Q = torch.linalg.eigh(mat)
            w = torch.clamp(w, min=eps)
            mat_spd = (Q * w) @ Q.t()
            return torch.linalg.cholesky(mat_spd)


def _safe_spd_sqrtm(mat: torch.Tensor) -> torch.Tensor:
    mat = (mat + mat.t()) * 0.5
    dmean = float(mat.diag().abs().mean().item())
    eps = 1e-6 * (dmean if dmean > 0.0 else 1.0)
    w, Q = torch.linalg.eigh(mat)
...
@torch.no_grad()
def _profile_whitening_mats_efficient(
    model_name: str,
    model: nn.Module,
    calib_loader,
    dev: str,
    ...
    """
    Memory-lean whitening profiling:
      - accumulate second moments in float32/float64 (configurable) instead of always float64
      - register hooks only for transformer-layer projection linears by default
      - use in-place addmm_ to avoid large temporary allocations

    Returns profiling_mat[layer_idx][linear_name] = whitening factor (Cholesky or SPD sqrt).
    """
    wf = str(whitening_factorization or "cholesky").strip().lower()
    if wf not in ("cholesky", "svd"):
        raise ValueError(f"Unsupported whitening_factorization: {whitening_factorization}")
    ...
    msg = (
        "[Profile] Whitening stats: raw X^T X (compat)"
        if compat_whitening
        else "[Profile] Whitening stats: centered covariance"
    )
    print(f"{msg}; stats_dtype={str(stats_dtype).replace('torch.', '')} only_supported={bool(only_supported)} wf={wf}")

    model = model.to(dev)
    model.eval()

    # Collect target Linear modules (avoid profiling every Linear in the full model).
    ...
    for li in range(len(layers)):
        ...
        for name, mod in subset.items():
            if not isinstance(mod, nn.Linear):
                continue
            if only_supported and (not _is_supported_proj_name(name)):
                continue
    ...
    def hook(module, input, output):
        try:
            ...
            x = x.to(device=dev, dtype=stats_dtype)
            if compat_whitening:
                module._acc.addmm_(x.t(), x, beta=1.0, alpha=1.0)
            else:
                module._second.addmm_(x.t(), x, beta=1.0, alpha=1.0)
                module._mean.add_(x.sum(dim=0))
                module._count += int(x.shape[0])
        except Exception:
            return
        finally:
            # Don't keep large outputs alive due to Python references.
            try:
                del output
            except Exception:
                pass
    ...
    for _name, mod in targets:
        in_f = int(mod.weight.shape[1]) if hasattr(mod, "weight") and torch.is_tensor(mod.weight) else int(mod.in_features)
        if compat_whitening:
            mod._acc = torch.zeros((in_f, in_f), dtype=stats_dtype, device=dev)
        else:
            mod._second = torch.zeros((in_f, in_f), dtype=stats_dtype, device=dev)
            mod._mean = torch.zeros((in_f,), dtype=stats_dtype, device=dev)
            mod._count = 0
    ...
    try:
        for batch in tqdm(calib_loader):
            if isinstance(batch, dict):
                ...
                # Fallback: allow list/tuple batches that are already model-ready.
                model(**batch)
    finally:
        for h in handles:
            try:
    ...
    # Offload model + stats to CPU before factorization to keep peak GPU memory low.
    model = model.cpu()
    for li in range(len(layers)):
        subset = find_layers(layers[li])
        for name, mod in subset.items():
            ...
            if only_supported and (not _is_supported_proj_name(name)):
                continue
            if compat_whitening and hasattr(mod, "_acc") and torch.is_tensor(getattr(mod, "_acc", None)):
                mod._acc = mod._acc.cpu()
            elif (not compat_whitening) and hasattr(mod, "_second") and torch.is_tensor(getattr(mod, "_second", None)):
                mod._second = mod._second.cpu()
                mod._mean = mod._mean.cpu()

    profiling_mat: Dict[int, Dict[str, torch.Tensor]] = {}
    print("[Profile] Start whitening factorization...")
    for li in tqdm(range(len(layers))):
        ...
        for name, mod in subset.items():
            ...
            if only_supported and (not _is_supported_proj_name(name)):
                continue
            if compat_whitening:
                if not hasattr(mod, "_acc") or (getattr(mod, "_acc", None) is None):
                    continue
                raw_xtx = mod._acc
                mat = raw_xtx.to(dev, dtype=torch.float32) if raw_xtx.dtype != torch.float32 else raw_xtx.to(dev)
                if wf == "svd":
                    scaling = _safe_spd_sqrtm(mat)
                else:
                    scaling = _safe_spd_cholesky(mat)
                layer_profile[name] = scaling.to(dtype=torch.float32).cpu()
                mod._acc = None
                del raw_xtx, mat, scaling
            else:
                if not hasattr(mod, "_second") or (getattr(mod, "_second", None) is None):
                    continue
                second = mod._second
                mean = mod._mean
                count = max(int(getattr(mod, "_count", 0)), 1)
                second = second.to(dev, dtype=torch.float32) if second.dtype != torch.float32 else second.to(dev)
                mean = mean.to(dev, dtype=torch.float32) if mean.dtype != torch.float32 else mean.to(dev)
                cov = second / float(count) - torch.outer(mean / float(count), mean / float(count))
                cov = (cov + cov.t()) * 0.5
                if wf == "svd":
                    scaling = _safe_spd_sqrtm(cov)
                else:
                    scaling = _safe_spd_cholesky(cov)
                layer_profile[name] = scaling.to(dtype=torch.float32).cpu()
                mod._second = None
                mod._mean = None
                mod._count = 0
                del second, mean, cov, scaling

            if str(dev).startswith("cuda"):
                torch.cuda.empty_cache()
        profiling_mat[int(li)] = layer_profile
...
def _masked_kl_div(
    ...
    """
    Token-level KL(teacher || student) with optional boolean mask over positions.
    Expects logits with shape (..., vocab). Returns a scalar.
    """
    ...
    t_prob = t_logp.exp()
    kl_pos = (t_prob * (t_logp - s_logp)).sum(dim=-1)  # (...,)
    if mask is not None:
        try:
            mask = mask.to(dtype=torch.bool, device=kl_pos.device)
...
def _mcq_rank_scores(
    ...
    *,
    ...
    """
    Return (scores_sum, scores_mean) for MCQ option-ranking without applying a hard-label loss.
    Shapes: (B, O) for both.
    """
    bsz, nopt, opt_len = options_ids.shape
    tau = max(1e-6, float(tau))
    ...
    labels = input_ids.clone()
    labels[:, :prompt_len] = -100
    labels[:, prompt_len:][opt_flat == int(pad_id)] = -100
    ...
    scores_sum = (-(nll_sum).reshape(bsz, nopt)) / tau
    tok_cnt = (shift_labels != -100).to(dtype=torch.float32).sum(dim=1).clamp(min=1.0)
    scores_mean = (-(nll_sum / tok_cnt).reshape(bsz, nopt)) / tau_mean
    if option_mask is not None:
        valid = option_mask.to(device=scores_sum.device, dtype=torch.bool)
        scores_sum = scores_sum.masked_fill(~valid, torch.finfo(scores_sum.dtype).min)
        scores_mean = scores_mean.masked_fill(~valid, torch.finfo(scores_mean.dtype).min)
...
def train_act_lora(
    ...
    mcq_pad_id: int = 0,
    mem_prof: Optional[CudaMemProfiler] = None,
    mem_log_every: int = 0,
    teacher_model: Optional[nn.Module] = None,
    kd_weight: float = 0.0,
    kd_temperature: float = 2.0,
    kd_on_unmasked_only: bool = True,
    kd_mcq_weight: Optional[float] = None,
    kd_disable_mcq: bool = False,
):
    stats: Dict[str, Any] = {"steps": 0, "tokens": 0, "sec": 0.0, "tokens_per_sec": None}
    if not params:
    ...
    if kd_mcq_weight is None:
        kd_mcq_w = float(kd_w)
    else:
        kd_mcq_w = float(kd_mcq_weight)
    amp_dtype = torch.bfloat16 if (str(device).startswith("cuda") and torch.cuda.is_bf16_supported()) else torch.float16
    step = 0
    t_train0 = time.perf_counter()
    if mem_prof is not None and getattr(mem_prof, "enabled", False):
        try:
            mem_prof.set_static_sizes(model=model, trainable_params=params)
        except Exception:
            pass
        try:
            mem_prof.snapshot("train_act_lora:start", model=model, trainable_params=params, optimizer=opt)
        except Exception:
            pass
    ...
    for ep in range(epochs):
        ...
        for batch in dataloader:
            ...
            with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=str(device).startswith("cuda")):
                if isinstance(batch, dict):
                    ...
                    if kind == "mcq_rank":
                        ...
                        try:
                            step_tokens = int(prompt_ids.numel()) * int(options_ids.shape[1]) + int(options_ids.numel())
                        except Exception:
                            step_tokens = 0
                        ...
                        if mcq_rank_tau_mean is not None:
                            try:
                                ...
                                tau_mean = None
                        # Compute student scores once (used for loss and optional distillation).
                        loss = mcq_rank_loss(
                            model=model,
                        ...
                        if (teacher_model is not None) and (kd_mcq_w > 0.0) and (not bool(kd_disable_mcq)):
                            try:
                                s_scores_sum, s_scores_mean = _mcq_rank_scores(
                                    model=model,
                                    prompt_ids=prompt_ids,
                                    options_ids=options_ids,
                                    pad_id=int(mcq_pad_id),
                                ...
                                with torch.no_grad():
                                    t_scores_sum, t_scores_mean = _mcq_rank_scores(
                                        model=teacher_model,
                                        prompt_ids=prompt_ids.to(next(teacher_model.parameters()).device),
                                        options_ids=options_ids.to(next(teacher_model.parameters()).device),
                                        pad_id=int(mcq_pad_id),
                                        tau=float(mcq_rank_tau),
                                        tau_mean=tau_mean,
                                        option_mask=option_mask.to(next(teacher_model.parameters()).device) if option_mask is not None else None,
                                    )
                                # Bring teacher scores back if teacher runs on a different device.
                                t_scores_sum = t_scores_sum.to(device=s_scores_sum.device)
                                t_scores_mean = t_scores_mean.to(device=s_scores_mean.device)
                                # Distill both sum-logprob and mean-logprob option distributions.
                                kd_sum = _masked_kl_div(
                                    student_logits=s_scores_sum,
                                    teacher_logits=t_scores_sum,
                                    mask=None,
                                    temperature=float(kd_temperature),
                                )
                                kd_mean = _masked_kl_div(
                                    student_logits=s_scores_mean,
                                    teacher_logits=t_scores_mean,
                                    mask=None,
                                    temperature=float(kd_temperature),
                                )
                                loss = loss + (kd_mcq_w * float(loss_w)) * (kd_sum + float(mcq_rank_mean_weight) * kd_mean)
                            except Exception:
                                pass
                        ...
                        loss = loss * loss_w
                        if (teacher_model is not None) and (kd_w > 0.0):
                            try:
                                with torch.no_grad():
                                    t_inp = inp.to(next(teacher_model.parameters()).device)
                                    t_logits = teacher_model(input_ids=t_inp, use_cache=False).logits.to(device=student_logits.device)
                                s_shift = student_logits[:, :-1, :].contiguous()
                                t_shift = t_logits[:, :-1, :].contiguous()
                                shift_labels = tar[:, 1:].contiguous()
                                mask = (shift_labels != -100) if bool(kd_on_unmasked_only) else None
                                kd_loss = _masked_kl_div(s_shift, t_shift, mask=mask, temperature=float(kd_temperature))
                                loss = loss + (kd_w * float(loss_w)) * kd_loss
                            except Exception:
                                pass
                    else:
            ...
            loss.backward()
            try:
                stats["tokens"] = int(stats.get("tokens", 0)) + int(step_tokens)
            except Exception:
                pass
            ...
            if (mem_prof is not None) and getattr(mem_prof, "enabled", False) and int(mem_log_every) > 0 and (step % int(mem_log_every) == 0):
                try:
                    mem_prof.snapshot(
                        f"train_act_lora:step{step}",
                        model=model,
                        trainable_params=params,
                        optimizer=opt,
                        extra={"step": int(step), "epoch": int(ep + 1)},
                    )
                except Exception:
                    pass
            if (step % log_every) == 0:
        ...
        if train_steps is not None and step >= int(train_steps):
            break
    stats["sec"] = float(time.perf_counter() - t_train0)
    try:
        if float(stats["sec"]) > 0:
            stats["tokens_per_sec"] = float(int(stats.get("tokens", 0)) / float(stats["sec"]))
    except Exception:
        stats["tokens_per_sec"] = None
    if mem_prof is not None and getattr(mem_prof, "enabled", False):
        try:
            stats["mem_end"] = mem_prof.snapshot(
                "train_act_lora:end",
                model=model,
                trainable_params=params,
                optimizer=opt,
                extra={"steps": int(stats.get("steps", 0)), "tokens": int(stats.get("tokens", 0)), "sec": float(stats.get("sec", 0.0))},
            )
        except Exception:
            pass
    try:
...
def train_act_lora_full_seq(
    ...
    if mem_prof is not None and getattr(mem_prof, "enabled", False):
        try:
            ...
            pass
        try:
            mem_prof.snapshot("train_act_lora_full:start", model=model, trainable_params=params, optimizer=opt)
        except Exception:
            pass
    def _report_stats(tag: str, loss_val):
        with torch.no_grad():
    ...
    for ep in range(epochs):
        ...
        for batch in dataloader:
            ...
            labels = inp
            try:
                stats["tokens"] = int(stats.get("tokens", 0)) + int(inp.numel())
            except Exception:
                pass
            with torch.autocast(device_type="cuda", dtype=amp_dtype, enabled=str(device).startswith("cuda")):
                ...
                if (teacher_model is not None) and (kd_w > 0.0):
                    try:
                        with torch.no_grad():
                            t_inp = inp.to(next(teacher_model.parameters()).device)
                            t_logits = teacher_model(input_ids=t_inp, use_cache=False).logits.to(device=out.logits.device)
                        s_shift = out.logits[:, :-1, :].contiguous()
                        t_shift = t_logits[:, :-1, :].contiguous()
                        kd_loss = _masked_kl_div(s_shift, t_shift, mask=None, temperature=float(kd_temperature))
                        loss = loss + (kd_w * float(loss_w)) * kd_loss
                    except Exception:
                        pass
            if not torch.isfinite(loss):
            ...
            if (mem_prof is not None) and getattr(mem_prof, "enabled", False) and int(mem_log_every) > 0 and (step % int(mem_log_every) == 0):
                try:
                    mem_prof.snapshot(
                        f"train_act_lora_full:step{step}",
                        model=model,
                        trainable_params=params,
                        optimizer=opt,
                        extra={"step": int(step), "epoch": int(ep + 1)},
...
def run_activation_lora(
    ...
    whitening_factorization: str = "cholesky",
    whitening_stats_dtype: str = "float32",
    whitening_profile_all_linears: bool = False,
    ...
    profile_mem_every: int = 0,
    # Stage-2 knowledge distillation (optional)
    kd_teacher_model: Optional[str] = None,
    kd_teacher_device: str = "cpu",
    kd_teacher_dtype: Optional[str] = None,
    kd_weight: float = 0.0,
    kd_temperature: float = 2.0,
    kd_on_unmasked_only: bool = True,
    kd_mcq_weight: Optional[float] = None,
    kd_disable_mcq: bool = False,
    kd_epochs: int = 0,
    kd_train_steps: Optional[int] = None,
    kd_lr: Optional[float] = None,
):
    """
    Pipeline:
    ...
    timing: Dict[str, Any] = {
        ...
        "whitening_stats_dtype": str(whitening_stats_dtype),
        "whitening_profile_all_linears": bool(whitening_profile_all_linears),
        ...
        "profile_mem": bool(profile_mem),
        "profile_mem_every": int(profile_mem_every),
        "kd_teacher_model": kd_teacher_model,
        "kd_teacher_device": str(kd_teacher_device),
        "kd_teacher_dtype": kd_teacher_dtype,
        "kd_weight": float(kd_weight),
        "kd_temperature": float(kd_temperature),
        "kd_on_unmasked_only": bool(kd_on_unmasked_only),
        "kd_mcq_weight": None if kd_mcq_weight is None else float(kd_mcq_weight),
        "kd_disable_mcq": bool(kd_disable_mcq),
        "kd_epochs": int(kd_epochs),
        "kd_train_steps": None if kd_train_steps is None else int(kd_train_steps),
        "kd_lr": None if kd_lr is None else float(kd_lr),
    ...
    model = None
    tokenizer = None
    mem_prof = CudaMemProfiler(enabled=bool(profile_mem), device=str(dev), sync=False)

    def _sync_cuda() -> None:
        if not getattr(mem_prof, "enabled", False):
            return
        try:
            if mem_prof.device_index is None:
                torch.cuda.synchronize()
            else:
                torch.cuda.synchronize(int(mem_prof.device_index))
        except Exception:
            try:
                torch.cuda.synchronize()
            except Exception:
    ...
    def _mem_extra(tag: str, trainable_params=None, optimizer=None) -> Dict[str, Any]:
        if not getattr(mem_prof, "enabled", False):
            return {}
        rec = mem_prof.snapshot(tag, model=model, trainable_params=trainable_params, optimizer=optimizer, quiet=False)
        if not isinstance(rec, dict):
            return {}
        return {
            "mem_alloc_bytes": int(rec.get("alloc_bytes", 0)),
            "mem_reserved_bytes": int(rec.get("reserved_bytes", 0)),
            "mem_peak_alloc_bytes": int(rec.get("peak_alloc_bytes", 0)),
            "mem_peak_reserved_bytes": int(rec.get("peak_reserved_bytes", 0)),
            "mem_params_bytes": int(rec.get("params_bytes", 0)),
            "mem_trainable_params_bytes": int(rec.get("trainable_params_bytes", 0)),
            "mem_grads_bytes": int(rec.get("grads_bytes", 0)),
            "mem_optim_bytes": int(rec.get("optim_bytes", 0)),
            "mem_other_bytes": int(rec.get("other_bytes", 0)),
        }
    ...
    if (not force_recompress) and whitened_cache is not None and os.path.exists(whitened_cache):
        ...
        if not trust_whitened_cache:
            ...
            req_stats_dtype = str(whitening_stats_dtype).strip().lower()
            req_profile_all_linears = bool(whitening_profile_all_linears)
            meta_ok = True
            if isinstance(cache_meta, dict) and cache_meta:
                try:
                    ...
                    if (saved_mlp is None) or (abs(saved_mlp - req_mlp) > 1e-8):
                        meta_ok = False
                    saved_stats_dtype = cache_meta.get("whitening_stats_dtype", None)
                    if saved_stats_dtype is not None:
                        if str(saved_stats_dtype).strip().lower() != req_stats_dtype:
                            meta_ok = False
                    else:
                        # Older caches may not store this key; only invalidate when explicitly requesting non-default stats dtype.
                        if req_stats_dtype not in ("float32", "fp32", "f32"):
                            meta_ok = False
                    saved_profile_all = cache_meta.get("whitening_profile_all_linears", None)
                    if saved_profile_all is not None:
                        if bool(saved_profile_all) != req_profile_all_linears:
                            meta_ok = False
                    else:
                        # Older caches may not store this key; invalidate only when explicitly requesting full-linear profiling.
                        if req_profile_all_linears:
                            meta_ok = False
    ...
    if getattr(mem_prof, "enabled", False):
        try:
            mem_prof.set_static_sizes(model=model)
        except Exception:
            pass
        try:
            _sync_cuda()
            mem_prof.reset_peak()
            _mem_extra("model:on_device")
        except Exception:
            pass
    try:
        model.seqlen = seqlen
    ...
    if cache_loaded:
        ...
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        _stage("build_whitening_data", time.perf_counter() - t0, **_mem_extra("build_whitening_data"))
        if profile_dev != dev:
        ...
        if getattr(mem_prof, "enabled", False):
            ...
            mem_prof.reset_peak()
        t0 = time.perf_counter()
        stats_dtype = _dtype_from_string(whitening_stats_dtype, default=torch.float32)
        profiling_mat = _profile_whitening_mats_efficient(
            model_name=model_id,
            model=model,
            calib_loader=cali_white_data,
            dev=profile_dev,
            whitening_factorization=wf,
            stats_dtype=stats_dtype,
            only_supported=(not whitening_profile_all_linears),
        )
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        _stage("profile_svdllm", time.perf_counter() - t0, whitening_factorization=wf, **_mem_extra("profile_svdllm"))
        ...
        if enable_adarank:
            ...
            _stage(
                ...
                adarank_max_target_scale=float(adarank_max_target_scale),
                adarank_relative_gain=bool(adarank_relative_gain),
                **_mem_extra("apply_whitening_adarank"),
            )
        else:
            ...
            if getattr(mem_prof, "enabled", False):
                _sync_cuda()
            _stage("apply_whitening_hetero", time.perf_counter() - t0, attn_keep_ratio=float(attn_ratio), mlp_keep_ratio=float(mlp_ratio), **_mem_extra("apply_whitening_hetero"))
        ...
        if whitened_cache is not None:
            ...
            if getattr(mem_prof, "enabled", False):
                _sync_cuda()
            _stage("save_whitened_cache", time.perf_counter() - t0, path=whitened_cache, **_mem_extra("save_whitened_cache"))
            model = model.to(dev)
    ...
    if not skip_eval:
        ...
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        _eval_stage("ppl_post_whitening", time.perf_counter() - t0, **_mem_extra("ppl_post_whitening"))
    ...
    if getattr(mem_prof, "enabled", False):
        _sync_cuda()
    _stage("attach_activation_lora", time.perf_counter() - t0, lora_rank=int(lora_rank), lora_alpha=float(lora_alpha), **_mem_extra("attach_activation_lora", trainable_params=lora_params))
    model = model.to(dev)
    if getattr(mem_prof, "enabled", False):
        try:
            mem_prof.set_static_sizes(model=model, trainable_params=lora_params)
        except Exception:
            pass
    if not lora_params:
    ...
    if getattr(mem_prof, "enabled", False):
        _sync_cuda()
    _stage("build_lora_loader_base", time.perf_counter() - t0, nsamples=int(lora_num), dataset=str(dataset), **_mem_extra("build_lora_loader_base", trainable_params=lora_params))
    ...
    if mix_buckets:
        ...
        default_props = {"LM": 0.30, "INST": 0.40, "MCQ": 0.15, "MATH": 0.15}
        ...
        # 1) LM bucket
        lm_names = [n.strip() for n in (bucket_lm_datasets.split(",") if bucket_lm_datasets else [dataset]) if n.strip()]
        lm_names = _dedup_names(lm_names)
        lm_batches_all: List = []
        lm_counts: Dict[str, int] = {}
        lm_ds_budget = _split_budget(lm_names, bucket_budget["LM"])

        def _lm_text(ex: Any) -> str:
        ...
        for lm_name in lm_names:
            lname = _norm_name(lm_name)
            budget = int(lm_ds_budget.get(lm_name, 0))
            if budget <= 0:
                continue
            if lname in ("c4", "c4_stream", "allenai/c4"):
                try:
                    from datasets import load_dataset
                except Exception as e:
                ...
                lm_loader = None
                try:
                    got = get_loaders(lname, nsamples=budget, seed=seed, seqlen=seqlen, tokenizer=tokenizer)
                    if got is not None:
                        lm_loader, _ = got
                except Exception:
                    lm_loader = None
                if lm_loader is not None:
                    ...
                    # Generic streaming fallback for HF text corpora (e.g., HuggingFaceFW/fineweb-edu).
                    try:
                        from datasets import load_dataset
                    except Exception as e:
                        raise RuntimeError(f"datasets library required to stream LM dataset {lm_name}: {e}")
                    try:
                        _ok = callable(tokenizer)
                    except Exception:
                        _ok = False
                    if not _ok or isinstance(tokenizer, bool):
                        tokenizer = _ensure_tokenizer(None, model_id, hf_token=hf_token)
                    import itertools
                    rr = random.Random(int(sft_seed) + 7)
                    try:
                        ds_path, ds_cfg = _split_hf_dataset_spec(lm_name)
                        if ds_cfg:
                            stream = load_dataset(ds_path, ds_cfg, split="train", streaming=True)
                        else:
                            stream = load_dataset(ds_path, split="train", streaming=True)
                    except Exception:
                        ds_path, ds_cfg = _split_hf_dataset_spec(lname)
                        if ds_cfg:
                            stream = load_dataset(ds_path, ds_cfg, split="train", streaming=True)
                        else:
                            stream = load_dataset(ds_path, split="train", streaming=True)
                    try:
                        stream = stream.shuffle(seed=int(sft_seed), buffer_size=10_000)
                    except Exception:
                        pass
                    seqs: List[torch.Tensor] = []
                    for ex in itertools.islice(iter(stream), int(c4_stream_train_docs)):
                    ...
                    for kidx in range(0, len(seqs), max(1, train_batch_size)):
                        ...
                        lm_batches_all.append((inp, tar))
                        added += 1
                    lm_counts[lm_name] = lm_counts.get(lm_name, 0) + added
        ...
        if inst_names:
            ...
            def _get_str(dp: Any, keys: List[str]) -> str:
            ...
            def _maybe_shuffle(split_obj, seed_val: int):
                try:
                    return split_obj.shuffle(seed=seed_val)
                except TypeError:
                    # IterableDataset requires buffer_size for shuffle
                    try:
                        return split_obj.shuffle(seed=seed_val, buffer_size=10_000)
                    except Exception:
                        return split_obj
                except Exception:
                    return split_obj

            def _load_train_split(ds_path: str, prefer_streaming: bool = False):
                path, cfg = _split_hf_dataset_spec(ds_path)
                if prefer_streaming:
                    try:
                        if cfg:
                            return load_dataset(path, cfg, split="train", streaming=True)
            ...
            def _inst_from_sharegpt(dp: Any):
                if not isinstance(dp, dict):
                    try:
                        dp = dict(dp)
                    except Exception:
                        return None
                conv = dp.get("conversations") or dp.get("messages") or dp.get("conversation") or dp.get("chat")
                ...
                def _role(m: Any) -> str:
                    if not isinstance(m, dict):
                        try:
                            ...
                            return ""
                    r = m.get("from") or m.get("role") or m.get("speaker") or m.get("author") or ""
                    r = str(r).strip().lower()
                    if r in ("human", "user", "prompt"):
                        return "user"
                    if r in ("gpt", "assistant", "bot", "model", "chatgpt"):
                        return "assistant"
                    if r in ("system",):
                        return "system"
                    return r
                ...
                for m in conv:
                    if _role(m) == "system":
                        system_msg = _content(m)
                        if system_msg:
                            break

                a_idx = None
                for idx in range(len(conv) - 1, -1, -1):
                    if _role(conv[idx]) == "assistant":
                        a_idx = idx
                        break
                if a_idx is None:
                    return None
                ...
                for idx in range(a_idx - 1, -1, -1):
                    if _role(conv[idx]) == "user":
                        u_idx = idx
                        break
                if u_idx is None:
                    return None

                instr = _content(conv[u_idx])
                out = _content(conv[a_idx])
                if not instr or not out:
                    return None
                if system_msg:
                    instr = (system_msg + "\n" + instr).strip()
                return {"instruction": instr, "input": None, "output": out}

            def _normalize_inst_record(ds_lname: str, dp: Any):
                rec = _inst_from_sharegpt(dp)
                if rec is not None:
                    return rec
                # Orca-style single-turn
                system_msg = _get_str(dp, ["system_prompt", "system", "system_message", "system_instruction"])
                instr = _get_str(dp, ["instruction", "Instruction", "question", "prompt", "query", "task"])
                inp = _get_str(dp, ["input", "context", "passage"])
                out = _get_str(dp, ["output", "response", "answer", "completion"])
                if system_msg and instr:
                    instr = (system_msg + "\n" + instr).strip()
                if not instr or not out:
                    return None
                return {"instruction": instr, "input": (inp if inp else None), "output": out}
            ...
            for name in inst_names:
                ...
                try:
                    if lname in ("yahma/alpaca-cleaned", "tatsu-lab/alpaca", "alpaca", "alpaca-cleaned"):
                        prompter = Prompter("alpaca")
                        ds_path, ds_cfg = _split_hf_dataset_spec(name)
                        ds = load_dataset(ds_path, ds_cfg) if ds_cfg else load_dataset(ds_path)
                        train_split = ds["train"].shuffle(seed=sft_seed)
                        for i, dp in enumerate(train_split):
                            if i >= take:
                                break
                            try:
                                ...
                                continue
                    elif lname in ("open-orca/slimorca", "openorca/slimorca", "slimorca", "open_orca/slimorca"):
                        prompter = Prompter("alpaca")
                        split = _load_train_split(name, prefer_streaming=True)
                        split = _maybe_shuffle(split, int(sft_seed))
                        for i, dp in enumerate(split):
                            if i >= take:
                                break
                            rec = _normalize_inst_record(lname, dp)
                            if rec is None:
                                continue
                            try:
                                inp, lab = _tokenize_prompt(prompter, rec)
                                pairs.append((inp, lab))
                            except Exception:
                                continue
                    elif lname in ("teknium/openhermes-2.5", "openhermes-2.5", "openhermes2.5", "openhermes"):
                        prompter = Prompter("alpaca")
                        split = _load_train_split(name, prefer_streaming=True)
                        split = _maybe_shuffle(split, int(sft_seed))
                        for i, dp in enumerate(split):
        ...
        def _mcq_from_race(dp):
            art = str(dp.get("article", "") or dp.get("passage", "")).strip()
            q = str(dp.get("question", "")).strip()
            opts = dp.get("options") or dp.get("choices") or []
            if not isinstance(opts, list):
                opts = []
            opts = [str(x).strip() for x in opts if str(x).strip()]
            if len(opts) < 2:
                return None
            ans = dp.get("answer", dp.get("label", ""))
            labels = [chr(ord("A") + i) for i in range(len(opts))]
            corr = _label_to_idx(ans, labels)
            if corr < 0 or corr >= len(opts):
                return None
            if art:
                prompt = f"Article: {art}\nQuestion: {q}\nAnswer:"
            else:
                prompt = f"Question: {q}\nAnswer:"
            return prompt, opts, corr

        def _mcq_from_sciq(dp, seed_val: int, idx: int):
            q = str(dp.get("question", "")).strip()
            support = str(dp.get("support", "") or dp.get("context", "")).strip()
            correct = str(dp.get("correct_answer", "") or dp.get("answer", "")).strip()
            distractors = dp.get("distractors")
            ...
            # Deterministic shuffle to reduce option-position bias
            rr = random.Random(int(seed_val) + 1337 + int(idx))
            perm = list(range(len(opts)))
            rr.shuffle(perm)
            opts = [opts[i] for i in perm]
            corr = perm.index(0)
            prompt = f"Context: {support}\nQuestion: {q}\nAnswer:" if support else f"Question: {q}\nAnswer:"
            return prompt, opts, corr

        def _mcq_from_social_iqa(dp):
            ctx = str(dp.get("context", "") or dp.get("ctx", "")).strip()
            q = str(dp.get("question", "")).strip()
            a = str(dp.get("answerA", "")).strip()
            b = str(dp.get("answerB", "")).strip()
            c = str(dp.get("answerC", "")).strip()
            opts = [a, b, c]
            if len([x for x in opts if x]) < 2:
                return None
            lab = dp.get("label", dp.get("answer", ""))
            corr = _safe_int(lab, default=-1)
            if corr in (1, 2, 3):
                corr = corr - 1
            if corr < 0 or corr >= len(opts):
                return None
            ...
            for name in mcq_names:
                ...
                try:
                    if lname == "hellaswag":
                        ...
                        # RACE: reading comprehension MCQ. Prefer combined configs when available.
                        train_split = None
                        try:
                            ...
                            try:
                                ...
                                ds_h = load_dataset("race", "high")
                                ds_m = load_dataset("race", "middle")
                                train_split = concatenate_datasets([ds_h["train"], ds_m["train"]])
                            except Exception:
                                ds = load_dataset("race")
                                try:
                                    train_split = ds["train"]
                                except Exception:
                                    train_split = ds
                        if train_split is None:
                            raise RuntimeError("RACE train split not found")
                        try:
                            train_split = train_split.shuffle(seed=sft_seed)
                        except Exception:
                            pass
                        for i, dp in enumerate(train_split):
                            if i >= take:
                                break
                            ex = _mcq_from_race(dp)
                            if ex is None:
                                continue
                            sample = _tokenize_mcq_sample(*ex)
                            if sample is not None:
                        ...
                        for i, dp in enumerate(train_split):
                            if i >= take:
                                break
                            ex = _mcq_from_sciq(dp, seed_val=int(sft_seed), idx=int(i))
                            if ex is None:
                                continue
                            sample = _tokenize_mcq_sample(*ex)
                            if sample is not None:
                                samples.append(sample)
                    elif lname in ("social_iqa", "social_i_qa"):
                        ds = load_dataset("social_i_qa")
                        train_split = ds["train"].shuffle(seed=sft_seed)
                        for i, dp in enumerate(train_split):
                            if i >= take:
                                break
                            ex = _mcq_from_social_iqa(dp)
                            if ex is None:
                                continue
                            sample = _tokenize_mcq_sample(*ex)
                            if sample is not None:
    ...
    def _should_run_stage(stage_epochs: int, stage_steps: Optional[int]) -> bool:
        try:
            ...
            pass
        try:
            return stage_steps is not None and int(stage_steps) > 0
        except Exception:
            return False

    def _norm_stage_epochs(stage_epochs: int, stage_steps: Optional[int]) -> int:
        ep = int(stage_epochs)
        if ep <= 0 and stage_steps is not None:
            try:
    ...
    def _train_one_stage(
        stage_name: str,
        stage_epochs: int,
        stage_steps: Optional[int],
        stage_lr: float,
        *,
        teacher: Optional[nn.Module] = None,
        kd_w: float = 0.0,
        kd_temp: float = 2.0,
        kd_on_unmasked: bool = True,
        kd_mcq_w: Optional[float] = None,
        kd_disable_mcq_flag: bool = False,
        extra_stage: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        nonlocal trained_any
        if not _should_run_stage(stage_epochs, stage_steps):
            return {}
        stage_epochs_eff = _norm_stage_epochs(stage_epochs, stage_steps)
        if stage_epochs_eff <= 0:
            return {}
        trained_any = True
        _maybe_enable_gradient_checkpointing(model, enable=bool(gradient_checkpointing))
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
            mem_prof.reset_peak()
        t0 = time.perf_counter()
        ...
        if effective_full_seq:
            train_stats = train_act_lora_full_seq(
                model,
                update_loader,
                lora_params,
                device=dev,
                epochs=int(stage_epochs_eff),
                train_steps=stage_steps,
                lr=float(stage_lr),
                log_every=log_every,
                mem_prof=mem_prof,
                mem_log_every=int(profile_mem_every),
                teacher_model=teacher,
                kd_weight=float(kd_w),
                kd_temperature=float(kd_temp),
            )
        else:
            train_stats = train_act_lora(
                model,
                update_loader,
                lora_params,
                device=dev,
                ...
                mcq_pad_id=int(train_mcq_pad_id),
                mem_prof=mem_prof,
                mem_log_every=int(profile_mem_every),
                teacher_model=teacher,
                kd_weight=float(kd_w),
                kd_temperature=float(kd_temp),
                kd_on_unmasked_only=bool(kd_on_unmasked),
                kd_mcq_weight=kd_mcq_w,
                kd_disable_mcq=bool(kd_disable_mcq_flag),
            )
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        mem_stage = {}
        try:
            mem_rec = train_stats.get("mem_end") if isinstance(train_stats, dict) else None
            if isinstance(mem_rec, dict):
                mem_stage = {
                    "mem_alloc_bytes": int(mem_rec.get("alloc_bytes", 0)),
                    "mem_reserved_bytes": int(mem_rec.get("reserved_bytes", 0)),
                    "mem_peak_alloc_bytes": int(mem_rec.get("peak_alloc_bytes", 0)),
                    "mem_peak_reserved_bytes": int(mem_rec.get("peak_reserved_bytes", 0)),
                    "mem_params_bytes": int(mem_rec.get("params_bytes", 0)),
                    "mem_trainable_params_bytes": int(mem_rec.get("trainable_params_bytes", 0)),
                    "mem_grads_bytes": int(mem_rec.get("grads_bytes", 0)),
                    "mem_optim_bytes": int(mem_rec.get("optim_bytes", 0)),
                    "mem_other_bytes": int(mem_rec.get("other_bytes", 0)),
                }
        except Exception:
            mem_stage = {}
        rec_extra: Dict[str, Any] = {
            ...
            "epochs": int(stage_epochs_eff),
            "train_steps": None if stage_steps is None else int(stage_steps),
            "lr": float(stage_lr),
            "train_steps_done": None if not isinstance(train_stats, dict) else int(train_stats.get("steps", 0)),
            "train_tokens": None if not isinstance(train_stats, dict) else int(train_stats.get("tokens", 0)),
            "train_tokens_per_sec": None
            if not isinstance(train_stats, dict)
            else (None if train_stats.get("tokens_per_sec", None) is None else float(train_stats.get("tokens_per_sec"))),
        }
        if extra_stage:
            rec_extra.update(extra_stage)
        _stage(stage_name, time.perf_counter() - t0, **rec_extra, **mem_stage)
        return train_stats

    # Stage-1: clean finetune (CE only)
    _train_one_stage(
        "train_activation_lora_clean",
        epochs,
        train_steps,
        lr,
        teacher=None,
        kd_w=0.0,
        extra_stage={"stage": "clean"},
    )

    # Stage-2: optional KD finetune (CE + KL)
    if _should_run_stage(kd_epochs, kd_train_steps):
        if kd_teacher_model is None or (not str(kd_teacher_model).strip()):
            raise ValueError("KD stage requested (kd_epochs/kd_train_steps) but kd_teacher_model is empty.")
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
            mem_prof.reset_peak()
        t_kd0 = time.perf_counter()
        teacher_spec = str(kd_teacher_model).strip()
        if teacher_spec.lower() in ("same", "self", "student"):
            teacher_spec = str(model_id)
        teacher_model = None
        if os.path.isfile(teacher_spec):
            ckpt = torch.load(teacher_spec, map_location="cpu")
            teacher_model = ckpt.get("model") if isinstance(ckpt, dict) and "model" in ckpt else ckpt
        else:
            teacher_model, _tok = get_model_from_huggingface(teacher_spec, hf_token=hf_token)
        if not isinstance(teacher_model, nn.Module):
            raise ValueError(f"Failed to load teacher model from: {teacher_spec}")
        try:
        ...
        try:
            ...
            pass
        if kd_teacher_dtype is not None:
            try:
                teacher_model = teacher_model.to(dtype=_dtype_from_string(kd_teacher_dtype, default=torch.float16))
            except Exception:
                pass
        tdev = str(kd_teacher_device) if kd_teacher_device is not None else "cpu"
        try:
            teacher_model = teacher_model.to(tdev)
        except Exception as e:
            print(f"[KD] Failed to move teacher to {tdev}: {e}. Falling back to CPU.")
        ...
        _stage(
            "load_kd_teacher",
            time.perf_counter() - t_kd0,
            teacher_model=str(teacher_spec),
            teacher_device=str(tdev),
            teacher_dtype=str(kd_teacher_dtype) if kd_teacher_dtype is not None else None,
            **_mem_extra("load_kd_teacher", trainable_params=lora_params),
        )
        kd_lr_eff = float(lr if kd_lr is None else kd_lr)
        _train_one_stage(
            "train_activation_lora_kd",
            kd_epochs,
            kd_train_steps,
            kd_lr_eff,
            teacher=teacher_model,
            kd_w=float(kd_weight),
            kd_temp=float(kd_temperature),
            kd_on_unmasked=bool(kd_on_unmasked_only),
            kd_mcq_w=kd_mcq_weight,
            kd_disable_mcq_flag=bool(kd_disable_mcq),
            extra_stage={
                "stage": "kd",
                "teacher_model": str(teacher_spec),
                "teacher_device": str(tdev),
        ...
        try:
            ...
            try:
                ...
                teacher_dev = None
            del teacher_model
            if teacher_dev is not None and str(teacher_dev).startswith("cuda"):
                torch.cuda.empty_cache()
        except Exception:
            pass

    if trained_any and (not skip_eval):
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
            mem_prof.reset_peak()
        ...
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        _eval_stage("ppl_post_lora", time.perf_counter() - t1, label=str(label), **_mem_extra("ppl_post_lora", trainable_params=lora_params))
    ...
    if save_path:
        ...
        if getattr(mem_prof, "enabled", False):
            _sync_cuda()
        _stage("save_checkpoint", time.perf_counter() - t0, path=save_path, **_mem_extra("save_checkpoint", trainable_params=lora_params))
    ...
    lora_stage_names = {"attach_activation_lora", "build_lora_loader_base", "train_activation_lora_clean", "train_activation_lora_kd"}
    ...
    if getattr(mem_prof, "enabled", False):
        timing["cuda_mem"] = {
            "run_peak_alloc_bytes": int(getattr(mem_prof, "run_peak_alloc_bytes", 0)),
            "run_peak_reserved_bytes": int(getattr(mem_prof, "run_peak_reserved_bytes", 0)),
            "records": list(getattr(mem_prof, "records", []) or []),
        }
        try:
            pa = float(int(getattr(mem_prof, "run_peak_alloc_bytes", 0)) / float(1024**3))
            pr = float(int(getattr(mem_prof, "run_peak_reserved_bytes", 0)) / float(1024**3))
            print(f"[Mem] RUN peak_alloc={pa:.2f}GiB peak_reserved={pr:.2f}GiB")
        except Exception:
            pass
...
def main():
    ...
    p.add_argument(
        "--whitening_stats_dtype",
        type=str,
        default="float32",
        choices=["float32", "float64"],
        help="Dtype for whitening stats accumulation during profiling. float32 uses ~2x less HBM than float64.",
    )
    p.add_argument(
        "--whitening_profile_all_linears",
        action="store_true",
        help="Profile all Linear modules (within transformer layers). Default profiles only q/k/v/o + MLP projections to save HBM.",
    )
    p.add_argument(
        "--attn_keep_ratio",
        type=float,
    ...
    p.add_argument(
        ...
        nargs="?",
        const=None,
        help="Optional HuggingFace token. If omitted (or empty), will fall back to env vars like HF_TOKEN / HUGGINGFACE_HUB_TOKEN.",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Enable HF gradient checkpointing during LoRA training to reduce peak HBM (trades compute for memory).",
    )
    ...
    # Optional Stage-2: teacher distillation (CE + KL)
    p.add_argument("--kd_teacher_model", type=str, default=None, help="Optional teacher model (HF id or local .pt) for a second-stage distillation run. Use 'same' to reuse --model.")
    p.add_argument("--kd_teacher_device", type=str, default="cpu", help="Teacher device for KD (cpu/cuda/cuda:0). For speed, put teacher on a different GPU.")
    p.add_argument("--kd_teacher_dtype", type=str, default=None, choices=["float16", "bfloat16", "float32"], help="Optional teacher dtype override (default: keep HF dtype).")
    p.add_argument("--kd_weight", type=float, default=0.0, help="KD weight for token-level KL (stage-2). Loss = CE + kd_weight * KL.")
    p.add_argument("--kd_temperature", type=float, default=2.0, help="KD temperature T (stage-2).")
    p.add_argument("--kd_on_all_tokens", action="store_true", help="Compute KD on all tokens (including masked prompt tokens for SFT). Default: only unmasked tokens (labels!=-100).")
    p.add_argument("--kd_mcq_weight", type=float, default=None, help="Optional KD weight for MCQ option distribution (defaults to --kd_weight).")
    p.add_argument("--kd_disable_mcq", action="store_true", help="Disable MCQ KD even if MCQ bucket is present.")
    p.add_argument("--kd_epochs", type=int, default=0, help="Epochs for KD stage (0 disables stage-2).")
    p.add_argument("--kd_train_steps", type=int, default=None, help="Optional max KD optimization steps (stage-2).")
    p.add_argument("--kd_lr", type=float, default=None, help="KD stage learning rate (defaults to --lr).")
    p.add_argument("--kd_only", action="store_true", help="Skip clean stage and run only KD stage (requires --kd_teacher_model and kd_epochs/steps).")
    ...
    p.add_argument("--bucket_props", type=str, default="LM:0.3,INST:0.4,MCQ:0.15,MATH:0.15", help="Bucket sampling proportions, e.g., 'LM:0.3,INST:0.4,MCQ:0.15,MATH:0.15'.")
    p.add_argument(
        "--bucket_lm_datasets",
        type=str,
        default="c4_stream,wikitext2",
        help="Comma-separated LM datasets. Supports get_loaders names (wikitext2/ptb/c4) plus generic HF streaming text datasets (e.g., HuggingFaceFW/fineweb-edu).",
    )
    p.add_argument(
        "--bucket_inst_datasets",
        type=str,
        default="Open-Orca/SlimOrca",
        help="Comma-separated instruction SFT datasets (masked CE). Recommended: Open-Orca/SlimOrca or teknium/OpenHermes-2.5. MCQ datasets should go to --bucket_mcq_datasets.",
    )
    p.add_argument(
        "--bucket_mcq_datasets",
        type=str,
        default="race,sciq,social_iqa",
        help="Comma-separated MCQ datasets trained with option-ranking loss (sum logprob). Non-leak recommended: race,sciq,social_iqa (do NOT include your eval MCQ benchmarks).",
    )
    p.add_argument(
        "--bucket_math_datasets",
        type=str,
        default="gsm8k,aqua_rat",
        help="Comma-separated math datasets (supports 'gsm8k', 'mathqa', 'aqua_rat'). For non-leak, avoid 'mathqa' if it's in your eval suite.",
    )
    p.add_argument("--bucket_total_batches", type=int, default=None, help="Cap the number of mixed batches (defaults to available).")
    p.add_argument("--bucket_loss_weights", type=str, default="LM:1.0,INST:1.0,MCQ:0.8,MATH:1.0", help="Per-bucket loss weights.")
    ...
    p.add_argument(
        "--disable_non_leak_guard",
        action="store_true",
        help="Disable non-leak guardrails that prevent training on common eval MCQ benchmarks (hellaswag/piqa/winogrande/arc/openbookqa).",
    )
    ...
    p.add_argument("--list_bucket_datasets", action="store_true", help="Print supported per-bucket dataset adapters and exit.")
    ...
    p.add_argument(
        "--profile_mem",
        action="store_true",
        help="Profile CUDA memory (alloc/reserved/peaks) with a rough breakdown into params/grads/optimizer/other; logs per stage and optionally during training.",
    )
    p.add_argument(
        "--profile_mem_every",
        type=int,
        default=0,
        help="When --profile_mem is enabled, also log memory every N LoRA training steps (0 disables).",
    )
    ...
    if args.list_bucket_datasets:
        print("Bucket keys: LM / INST / MCQ / MATH")
        print("")
        print("[LM] Built-in: wikitext2, ptb, c4, c4_stream")
        print("     Also supports generic HF streaming text datasets (expects fields like text/content/document/article/body).")
        print("")
        print("[INST] Built-in adapters: alpaca (yahma/alpaca-cleaned, tatsu-lab/alpaca), Open-Orca/SlimOrca, teknium/OpenHermes-2.5, glue/cola, glue/sst2")
        print("       Generic fallback: ShareGPT-like conversations/messages; or instruction/input/output; or prompt/response/answer/completion.")
        print("")
        print("[MCQ] Built-in adapters: race, sciq, social_iqa")
        print("      Also supported (but blocked by default non-leak guard): hellaswag, piqa, winogrande*, arc_easy/arc_challenge, openbookqa.")
        print("")
        print("[MATH] Built-in adapters: gsm8k, aqua_rat, mathqa (mathqa is blocked by default non-leak guard).")
        sys.exit(0)
    ...
    # Non-leak guard: prevent accidentally training on common eval MCQ benchmarks.
    if args.mix_buckets and (not args.disable_non_leak_guard):
        banned_mcq = {
            "hellaswag",
            "piqa",
            "winogrande",
        ...
        def _split_names(s: Optional[str]) -> List[str]:
            return [x.strip() for x in (str(s).split(",") if s else []) if x.strip()]

        candidates = (
            _split_names(args.bucket_lm_datasets)
            + _split_names(args.bucket_inst_datasets)
            + _split_names(args.bucket_mcq_datasets)
            + _split_names(args.bucket_math_datasets)
        )
        bad = []
        for n in candidates:
            ln = str(n).strip().lower()
            if ln in banned_mcq or ln.startswith("winogrande"):
                bad.append(n)
        if bad:
            print(
                "[NonLeakGuard] Refusing to use eval MCQ benchmarks in training buckets: "
                + ", ".join(sorted(set(bad)))
            )
            print("[NonLeakGuard] Use non-leak MCQ datasets like: race,sciq,social_iqa, or pass --disable_non_leak_guard.")
            sys.exit(2)

    # KD stage sanity checks
    if args.kd_only:
        args.epochs = 0
        args.train_steps = 0
    kd_wants_run = False
    try:
        kd_wants_run = (int(args.kd_epochs) > 0) or (args.kd_train_steps is not None and int(args.kd_train_steps) > 0)
    except Exception:
        kd_wants_run = args.kd_train_steps is not None
    if kd_wants_run and (args.kd_teacher_model is None or (not str(args.kd_teacher_model).strip())):
        print("[KD] Refusing to run KD stage without --kd_teacher_model. Set --kd_teacher_model <hf_id|local_pt> or disable KD by setting --kd_epochs 0.")
        sys.exit(2)
    ...
    run_activation_lora(
        ...
        whitening_stats_dtype=args.whitening_stats_dtype,
        whitening_profile_all_linears=args.whitening_profile_all_linears,
        ...
        profile_mem_every=args.profile_mem_every,
        kd_teacher_model=args.kd_teacher_model,
        kd_teacher_device=args.kd_teacher_device,
        kd_teacher_dtype=args.kd_teacher_dtype,
        kd_weight=args.kd_weight,
        kd_temperature=args.kd_temperature,
        kd_on_unmasked_only=(not bool(args.kd_on_all_tokens)),
        kd_mcq_weight=args.kd_mcq_weight,
        kd_disable_mcq=args.kd_disable_mcq,
        kd_epochs=args.kd_epochs,
        kd_train_steps=args.kd_train_steps,
        kd_lr=args.kd_lr,
