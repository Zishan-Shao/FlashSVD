...
def _remove_unpicklable_hf_input_grads_hook(model: nn.Module) -> None:
    """
    transformers.PreTrainedModel.enable_input_require_grads() registers a locally-defined
    forward hook (make_inputs_require_grads) on the input embeddings. That closure is
    not picklable, causing torch.save(model) to fail.

    Remove it in a targeted way (avoid clearing all hooks, which may break accelerate's
    dispatch/offload hooks when device_map is used).
    """
    try:
        h = getattr(model, "_require_grads_hook", None)
    except Exception:
        h = None
    if h is not None:
        try:
    ...
    def _drop_bad_hooks(mod: nn.Module) -> None:
        for attr in ("_forward_hooks", "_forward_pre_hooks"):
            ...
            if not isinstance(hooks, dict) or not hooks:
                continue
            for k, fn in list(hooks.items()):
                try:
                    qn = str(getattr(fn, "__qualname__", ""))
                except Exception:
                    qn = ""
                if ("enable_input_require_grads" in qn) or ("make_inputs_require_grads" in qn):
                    try:
                        hooks.pop(k, None)
                    except Exception:
                        pass

    # Prefer input embeddings if available, but also scan the whole model as a fallback.
    try:
...
def run_activation_lora(
    ...
    if epochs > 0:
        ...
        _stage(
            ...
            train_tokens_per_sec=None if not isinstance(train_stats, dict) else (None if train_stats.get("tokens_per_sec", None) is None else float(train_stats.get("tokens_per_sec"))),
            **mem_stage,
        )
        if not skip_eval:
            if getattr(mem_prof, "enabled", False):
    ...
    if save_path:
        ...
        # Remove unpicklable HF hooks (commonly introduced by gradient checkpointing helpers)
        try:
            _remove_unpicklable_hf_input_grads_hook(model)
        except Exception:
            pass
...
def main():
    ...
    p.add_argument(
        ...
        type=str,
        default="mathqa",
        help="Comma-separated math datasets (supports 'gsm8k', 'mathqa', 'aqua_rat').",
    )
    ...
    if args.mix_buckets and (not args.disable_non_leak_guard):
        ...
        candidates = _split_names(args.bucket_mcq_datasets) + _split_names(args.bucket_inst_datasets)
        bad = []
        for n in candidates:
